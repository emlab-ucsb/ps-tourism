...
# ##-save file here for transfer to other computer
# datalist = list(transformed_stockdistrib=transformed_stockdistrib,stocklist=stocklist,MegaData_filtered_step2=MegaData_filtered_step2)
# saveRDS(datalist,here("data","distance-library","transfer_file_other_computer","seed_data.RData"))
# rm(list = ls())
# datalist<-readRDS(here("data","distance-library","transfer_file_other_computer","seed_data.RData"))
# datalist
...
# #hey<-readRDS(distmat_filenames[1]) %>% mutate(distance = distance/1000) %>% filter(source %in% stock_subset_i, sink %in% stock_subset_i, distance<=MegaData_filtered_step2$dispersal_distance_limit[stock_num])
# larvaeconnect_full <-foreach(i=1:length(distmat_filenames),.combine=rbind)%dopar%{
#   readRDS(distmat_filenames[i]) %>% mutate(distance = distance/1000)
# }
# 
# print(length(unique(larvaeconnect_full$sink))==length(unique(larvaeconnect_full$source)))
# #something is wrong!!!
# #sink is 149542, source is 147656.. so source file is wrong!!!
# 
# #How about the old connect matrix
# larvaeconnect_old <- readRDS(here("data", "distance-library", "merged_dist_matrix","merged_dist_matrix.rds")) 
# print(length(unique(larvaeconnect_old$sink))==length(unique(larvaeconnect_old$source)))
# 
source(here("scripts", "functions","func_stitch_connect_matrix.R"))

# # ###checking code if working
# nstock<-length(stocklist)
# # 
# stock_num<-stocklist[8]
...
# head(larvaeconnect_merged)
# print(length(unique(larvaeconnect_merged$sink))==length(unique(larvaeconnect_merged$source)))
# 
# ###checking code if working
# stock_num<-stocklist[1]
# stock_subset_i <- which(transformed_stockdistrib[,stock_num+5] > 0)
# larvaeconnect_merged <-foreach(i=1:length(distmat_filenames),.combine=rbind)%dopar%{
#   readRDS(distmat_filenames[i]) %>% mutate(distance = distance/1000) %>% filter(source %in% stock_subset_i, sink %in% stock_subset_i, distance<=MegaData_filtered_step2$dispersal_distance_limit[stock_num]) %>% data.frame()
# }
# print(length(unique(larvaeconnect_merged$sink))==length(unique(larvaeconnect_merged$source)))
# 
# ###check using the old file
# #
...
# stock_num=11
# stock_subset_i <- which(transformed_stockdistrib[,stock_num+5] > 0)
# distance_mat_full_prop <- func_stitch_connect_matrix(distmat_filenames=distmat_filenames,stock_subset_i=stock_subset_i,MegaData_filtered_step2=MegaData_filtered_step2,stock_num=stock_num)
...
#   stocklist2 <- stocklist[28:821]
#   nstock<-length(stocklist2)
#   for (stock_num in stocklist2){
#     stock_subset_i <- which(transformed_stockdistrib[,stock_num+5] > 0)
#     distance_mat_full_prop_larvae <- func_stitch_connect_matrix(distmat_filenames=distmat_filenames,stock_subset_i=stock_subset_i,MegaData_filtered_step2=MegaData_filtered_step2,stock_num=stock_num)
...
for (i in 1:length(connect_matrix_nfiles)){
  ...
  print(dim(check)[1])
  #print(c(connect_matrix_nfiles[1],dim(check)[1]))
}
...
# stock_include<-c(1,3,5)#which(MegaData$INCLUDE==1)
...
mean(Delta_biomass[dive_suitability$cell_id]) #0.4359 increase in biomass density or from below's calculation, x% of base biomass.
...
mean(biomass_data$ratio_biom_divesite, na.rm=T) #on average, total biomass inside MPAs will increase by 113% vs. BAU
...
# b0<-transformed_stockdistrib %>% ggplot(aes(x=lon,y=lat,fill=Ratio_biomass)) + scale_fill_gradient(low = "white", high = "firebrick", na.value = NA,limits=c(1,max_ratio_biomass))+#scale_fill_viridis_c(limits = c(1, max(Ratio_biomass,na.rm=T)))+
#   theme(axis.title.x = element_blank(),axis.title.y = element_blank(), panel.background = element_blank())+
#   geom_raster()+geom_sf(data = land_shp_moll,fill="darkgrey", lwd = 0,  inherit.aes = F)+labs(title = "", fill = "Biomass ratio")
...
#!!! Those with no names perhaps are areas with no clear jurisdiction. Fix those? Or assume as 1 country. Let us assume for now as one country.
...
###STOP###
##exploring here plotting
plotme_test1<-normalized_per_pixel_delta_biodiv_benefit
plotme_test1$category <- "diversity"
plotme_test1 <- plotme_test1 %>% select(!cell_id)
colnames(plotme_test1) <- c("delta_diversity_or_biomassdensity","category")
plotme_test1$delta_diversity_or_biomassdensity<-plotme_test1$delta_diversity_or_biomassdensity/max(plotme_test1$delta_diversity_or_biomassdensity)
head(plotme_test1)

plotme_test2<- change_biomass_density_full %>% data.frame()
plotme_test2$category <- "biomass density"
colnames(plotme_test2) <- c("delta_diversity_or_biomassdensity","category")
plotme_test2$delta_diversity_or_biomassdensity<-plotme_test2$delta_diversity_or_biomassdensity/max(plotme_test2$delta_diversity_or_biomassdensity)

max(plotme_test2$delta_diversity_or_biomassdensity)

plotme_test <- rbind(plotme_test1,plotme_test2)

deltabio<-ggplot(plotme_test,aes(x=category,y=delta_diversity_or_biomassdensity,group_by(category)))+ geom_boxplot() + theme_classic()
deltabio
ggsave(here("figures","main","deltabio.jpg"),deltabio, width = 5, height = 5, units = "in")
